...
3. Use a Docker Compose command to start all of the component's Docker containers, will pull images if not already existing.
   ...
   - [Connector Note]: This example does not have the ids:connectorProxy entry in the connector configuration set. If a proxy is to be used, it must first be added to the conncetor configuration.
